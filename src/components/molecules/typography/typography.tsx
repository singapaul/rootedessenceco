...
export function TypographyH1({ children , className}: { children: any, className?:string }) {
  return (
    <h1 className={clsx('scroll-m-20 text-4xl font-extrabold lg:text-5xl', className)}>
      {children}
    </h1>
  );
}
...
export function TypographyH2({ children , className}: { children: any, className?:string }) {
  return (
    <h2 className={clsx("scroll-m-20 text-3xl font-semibold tracking-tight first:mt-0", className)}>
      {children}
    </h2>
  );
}
