  ...
  return (
      ...
      <section className="w-full flex flex-col items-center justify-evenly bg-orange-50 text-green-900 min-h-96">
        ...
        >
          ...
          {values.map((value) => {
            return (
              <TabsContent key={value.title} value={value.title} className="w-full self-center text-center max-w-[600px]">
                <TypographyP className="text-center">{value.description}</TypographyP>
              </TabsContent>
            );
          })}
        </Tabs>
